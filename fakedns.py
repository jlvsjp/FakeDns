...
class SOA(DNSResponse):
    def __init__(self, query, config_location):
        ...
        self.type = b"\x00\x06"
        # name server that was original or primary source for this zone
        self.mname = config.get(query.domain.decode(), "mname")
        # domain name which specified mailbox of person responsible for zone
        self.rname = config.get(query.domain.decode(), "rname")
        # 32-bit long version number of the zone copy
        self.serial = serial
        # 32-bit time interval before zone refresh
        self.refresh = config.getint(query.domain.decode(), "refresh")
        # 32-bit time interval before retrying failed refresh
        self.retry = config.getint(query.domain.decode(), "retry")
        # 32-bit time interval after which the zone is not authoritative
        self.expire = config.getint(query.domain.decode(), "expire")
        # The unsigned 32 bit minimum TTL for any RR from this zone.
        self.minimum = config.getint(query.domain.decode(), "minimum")
...
class NONEFOUND(DNSResponse):
    def __init__(self, query):
        ...
        self.data = b"\x00"
        logging.info(">> Built NONEFOUND response")
...
class RuleEngine2:
        ...
        with open(file_, 'r') as rulefile:
            ...
            for rule in rules:
                ...
                self.rule_list.append(
                    Rule(rule_type, domain, ips, rebinds, rebind_threshold))

                # increment the line number
                lineno += 1

            logging.info(">> Parsed %d rules from %s" %
                         (len(self.rule_list), file_))
    ...
    def match(self, query, addr):
        ...
        if args.noforward:
            logging.info(">> Don't Forward %s" % query.domain.decode())
            return NONEFOUND(query).make_packet()
        try:
            ...
            data = s.recv(1024)
            s.close()
            logging.info("Unmatched Request " + query.domain.decode())
            return data
        except socket.error as e:
            # We shouldn't wind up here but if we do, don't drop the request
            # send the client *something*
            logging.info(">> Error was handled by sending NONEFOUND")
            logging.info(e)
            return NONEFOUND(query).make_packet()
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='dns.log',
    ...
    if not os.path.isfile(path):
        logging.info('>> Please create a "dns.conf" file or specify a config path: '
    ...
    try:
        ...
        logging.info(
            ">> Could not start server -- is another program on udp:{0}?".format(port))
        exit(1)
